...
- Prevent firmware update spec change mid update
...
```
var (
	// Block disabling NTP as this is unsupported on switchd 1.0.9
	failNtpDisableRequests_switchd109 bool = true
	// Track firmware update state per switch
	switchFwUpdateState = make(map[string]*firmwareUpdateState, 2)
)

type firmwareUpdateState struct {
	targetVersion    string        // Version we want to update to
	completedVersion string        // Version we successfully completed update to
	updateChan       chan struct{} // Notification channel
}

// Used for fake switch firmware update
// Likely to be removed in the future as state should set externally
func initFwUpdateState(name string) {
	if switchFwUpdateState[name] == nil {
		switchFwUpdateState[name] = &firmwareUpdateState{}
	}
}


	// Initialize firmware update state map for each switch
	initFwUpdateState(hfSwitch.Name)
